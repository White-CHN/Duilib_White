...
namespace DuiLib
{
    ...
    CDuiControl* CDuiDlgBuilder::Create(STRINGorID xml, LPCTSTR type /*= NULL*/, IDialogBuilderCallback* pCallback /*= NULL*/, CDuiPaintManager* pManager /*= NULL*/, CDuiControl* pParent /*= NULL*/)
    {
        ...
        if(HIWORD(xml.m_lpstr) != NULL)
        {
            if(*(xml.m_lpstr) == _T('<'))
            {
                ...
                {
                    DUI_ERROR("this[0x%x] CDuiMarkup::Load[FALSE] m_lpstr[%s]", this, xml.m_lpstr);
                    return NULL;
                }
            }
            else
            {
                ...
                {
                    DUI_ERROR("this[0x%x] CDuiMarkup::LoadFromFile[FALSE] m_lpstr[%s]", this, xml.m_lpstr);
                    return NULL;
                }
            }
        }
        else
        {
            ...
            if(hInstence == NULL)
            {
                DUI_ERROR("this[0x%x] GetResourceDll[NULL] ", this);
                return NULL;
            }
            HRSRC hResource = ::FindResource(hInstence, xml.m_lpstr, type);
            if(hResource == NULL)
            {
                DUI_ERROR("this[0x%x] FindResource[NULL] hInstence[0x%x] m_lpstr[%s] type[%d]", this, hInstence, xml.m_lpstr, type);
                return NULL;
            }
            HGLOBAL hGlobal = ::LoadResource(hInstence, hResource);
            if(hGlobal == NULL)
            {
                DUI_ERROR("this[0x%x] LoadResource[NULL] hInstence[0x%x] hResource[0x%x]", this, hInstence, hResource);
                FreeResource(hResource);
                return NULL;
            }
            if(!m_xml.LoadFromMem((BYTE*)::LockResource(hGlobal), ::SizeofResource(hInstence, hResource)))
            {
                DUI_ERROR("this[0x%x] LoadFromMem[FALSE] hGlobal[0x%x] hInstence[0x%x] hResource[0x%x]", this, hGlobal, hInstence, hResource);
                ::FreeResource(hResource);
                return NULL;
            }
            ::FreeResource(hResource);
    ...
    CDuiControl* CDuiDlgBuilder::Create(CDuiControl* pParent)
    {
        ...
        if(!root.IsValid())
        {
            DUI_ERROR("this[0x%x] CDuiMarkupNode::IsValid[FALSE] ", this);
            return NULL;
        }
        if(m_pManager)
        {
    ...
    CDuiControl* CDuiDlgBuilder::Parse(CDuiMarkupNode* pRoot, CDuiControl* pParent)
    {
        ...
        for(CDuiMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling())
        {
            ...
            {
#ifdef _DEBUG
                DUI_TRACE("this[0x%x] 未知控件[%s]", this, pstrClass);
#else
                continue;
#endif
            }
