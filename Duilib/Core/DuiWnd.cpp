...
namespace DuiLib
{
    ...
    HWND CDuiWnd::GetHWND() const
    {
        if(!IsWindow(m_hWnd))
        {
            DUI_ERROR("this[0x%x] m_hWnd[0x%x]", this, m_hWnd);
            return NULL;
        }
        return m_hWnd;
    }
    ...
    HWND CDuiWnd::Create(HWND hWndParent, LPCTSTR lpWindowName, DWORD dwStyle, DWORD dwExStyle, int x /*= CW_USEDEFAULT*/, int y /*= CW_USEDEFAULT*/, int nWidth /*= CW_USEDEFAULT*/, int nHeight /*= CW_USEDEFAULT*/, HMENU hMenu /*= NULL*/)
    {
        ...
        {
            DUI_ERROR("this[0x%x] GetSuperClassName[%s] RegisterSuperClass[FALSE]", this, GetSuperClassName());
            return NULL;
        }
        if(GetSuperClassName() == NULL && !RegisterWindowClass())
        {
            DUI_ERROR("this[0x%x] GetSuperClassName[NULL] RegisterWindowClass[FALSE]", this);
            return NULL;
        }
        ...
        DUI_TRACE("this[0x%x] dwExStyle[0x%x] ClassName[%s] WindowName[%s] dwStyle[0x%x] x[%d] y[%d] nWidth[%d] nHeight[%d] hWndParent[0x%x] hInstance[0x%x] m_hWnd[0x%x]",
    ...
    HWND CDuiWnd::SubClass(HWND hWnd)
    {
        ...
        {
            DUI_ERROR("this[0x%x] SubclassWindow[NULL]", this);
            return NULL;
        }
        m_bSubclassed = TRUE;
        m_hWnd = hWnd;
        ...
        {
            DUI_ERROR("this[0x%x] SetWindowLongPtr[NULL]", this);
        }
        return m_hWnd;
    }
    ...
    BOOL CDuiWnd::CenterWindow()
    {
        ...
        {
            DUI_ERROR("this[0x%x] SetWindowPos[FALSE] m_hWnd[0x%x] xLeft[%d] yTop[%d] uFlag[0x%x]", this, m_hWnd, xLeft, yTop, SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE);
            return FALSE;
        }
        return TRUE;
    }
    ...
    BOOL CDuiWnd::ResizeClient(int cx /*= -1*/, int cy /*= -1*/)
    {
        ...
        {
            DUI_ERROR("this[0x%x] AdjustWindowRectEx[FALSE] m_hWnd[0x%x]", this, m_hWnd);
            return FALSE;
        }
        if(::SetWindowPos(m_hWnd, NULL, 0, 0, rc.right - rc.left, rc.bottom - rc.top, SWP_NOZORDER | SWP_NOMOVE | SWP_NOACTIVATE) == NULL)
        {
            DUI_ERROR("this[0x%x] SetWindowPos[FALSE] m_hWnd[0x%x] cx[%d] cy[%d] uFlag[0x%x]", this, m_hWnd, rc.right - rc.left, rc.bottom - rc.top, SWP_NOZORDER | SWP_NOMOVE | SWP_NOACTIVATE);
            return FALSE;
        }
        return TRUE;
    }
