...
namespace DuiLib {
	...
	BOOL DrawImage(HDC hDC, CDuiPaintManager* pManager, const RECT& rc, const RECT& rcPaint, const CDuiString& sImageName, \
		...
		do 
		{
			if( type == NULL ) {
				...
				if( CDuiPaintManager::GetResourceZip().IsEmpty() ) {
					...
					HZIP hz = NULL;
					if( CDuiPaintManager::IsCachedResourceZip() ) hz = (HZIP)CDuiPaintManager::GetResourceZipHandle();
					...
					if( res != 0x00000000 && res != 0x00000600) {
						delete[] pData;
						pData = NULL;
						if( !CDuiPaintManager::IsCachedResourceZip() ) CloseZip(hz);
						break;
					}
					if( !CDuiPaintManager::IsCachedResourceZip() ) CloseZip(hz);
				}
				...
				{
					dllinstance = CDuiPaintManager::GetResourceDll();
				}
	...
	static DWORD LoadImage2Memory(const STRINGorID &bitmap, LPCTSTR type,LPBYTE &pData)
	{
		...
		do 
		{
			...
			{
				HINSTANCE hDll = CDuiPaintManager::GetResourceDll();
	...
	BOOL CRenderEngine::DrawImageString(HDC hDC, CDuiPaintManager* pManager, const RECT& rcItem, const RECT& rcPaint, LPCTSTR pStrImage, LPCTSTR pStrModify, HINSTANCE instance)
	{
	...
	void CRenderEngine::DrawText(HDC hDC, CDuiPaintManager* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, int iFont, UINT uStyle)
	{
		ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
		if( pstrText == NULL || pManager == NULL ) return;
	...
	void CRenderEngine::DrawHtmlText(HDC hDC, CDuiPaintManager* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, RECT* prcLinks, CDuiString* sLinks, int& nLinkRects, UINT uStyle)
	{
	...
	HBITMAP CRenderEngine::GenerateBitmap(CDuiPaintManager* pManager, CDuiControl* pControl, RECT rc)
	{
		int cx = rc.right - rc.left;
		int cy = rc.bottom - rc.top;
	...
	SIZE CRenderEngine::GetTextSize( HDC hDC, CDuiPaintManager* pManager , LPCTSTR pstrText, int iFont, UINT uStyle )
	{
		SIZE size = {0,0};
		ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
